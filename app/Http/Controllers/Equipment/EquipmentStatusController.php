    ...
    public function update($id, Request $request)
    {
        ...
        if ($equipment->status === 'pending') {
            if (!$isManager && !$isAdmin) {
                return back()->withErrors(['status' => 'Only the manager can approve or reject this request.']);
            }
            ...
            $equipment->comment = $request->input('comment');
            $equipment->save();
            return redirect()->back()->with('success', 'Request approved. Sent to admin for delivery approval.');
        }
        ...
        if ($equipment->status === 'approved') {
            ...
            if ($validated['status'] === 'final_rejection') {
                $equipment->status = 'final_rejection';
                $equipment->admin_approver_id = $user->id;
                $equipment->comment = $request->input('comment');
                $equipment->save();
                return redirect()->back()->with('success', 'Request finally rejected.');
            }
            ...
            foreach ($items as $item) {
                ...
                $deliveryQty = (int) ($adminDelivery[$tid] ?? 0);
                if ($deliveryQty <= 0) {
                    continue;
                }
                $inventory = (int) WarehouseItem::where('product_template_id', $tid)->whereNull('recipient_id')->sum('quantity');
                if ($deliveryQty > $inventory) {
                    return back()->withErrors([
                        'admin_delivery_items' => 'Approved quantity for delivery cannot exceed inventory (' . $inventory . ') for product.',
                    ]);
                }
            }
            ...
            $equipment->status = 'final_approval';
            $equipment->admin_approver_id = $user->id;
            $equipment->save();
            return redirect()->back()->with('success', 'Request finally approved. Inventory deducted.');
        }
