...
const statusLabels: Record<string, string> = {
    pending: 'Pending',
    declined_by_manager: 'Declined by the manager',
    approved_by_manager: 'Approved by the manager (awaiting admin)',
...
export default function EquipmentShow() {
    ...
    useEffect(() => {
        if (data.status === 'approved_by_manager' && data.items?.length > 0 && data.canAdminAct) {
            const initial: Record<string, number> = {};
            data.items.forEach((item: any) => {
                const tid = String(item.product_template_id);
                const mgrApproved = data.manager_approved_items?.[tid] ?? 0;
                const inv = data.inventoryByTemplate?.[item.product_template_id] ?? 0;
                const maxQty = Math.min(mgrApproved, inv);
                initial[tid] = Math.min(item.quantity ?? 0, maxQty, mgrApproved);
            });
    ...
    const handleAdminStatus = async (newStatus: 'final_approval' | 'final_rejection') => {
        try {
            ...
            setShowModal(false);
            router.visit('/admin/product-requests');
        } catch (error: any) {
            setSuccessMessage(error?.response?.data?.message || 'Error.');
        } finally {
            setProcessing(false);
    ...
    const showInStock = data.isAdmin === true;
                    ...
                    <table className="mt-2 w-full rounded border text-sm text-gray-700">
                        <thead>
                            <tr className="bg-gray-100">
                                ...
                                {showInStock && <th className="p-2 text-left">In stock</th>}
                        ...
                        <tbody>
                            {items.map((item: any, idx: number) => {
                                ...
                                const mgrQty = managerApproved[tidStr] ?? 0;
                                const admQty = adminDelivery[tidStr] ?? 0;
                                const showAdminCol = canAdminAct || (adminDelivery && admQty > 0);
                                const managerApprovedDisplay =
                                    status === 'pending' || status === 'declined_by_manager' ? '—' : mgrQty;
                                return (
                                        ...
                                        <td className="p-2">
                                            {canManagerAct ? (
                                                <input
                                                    ...
                                                    min={0}
                                                    max={item.quantity}
                                                    value={managerApprovedQuantities[tidStr] ?? item.quantity ?? 0}
                                                    onChange={(e) =>
                                                ...
                                                managerApprovedDisplay
                                            )}
                                        </td>
                                        {showInStock && <td className="p-2">{inv}</td>}
                                        {showAdminCol && (
                                            <td className="p-2">
                                                {canAdminAct ? (
                                                    <input
